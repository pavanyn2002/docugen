...
export const changeRecordSchema = z
  .object({
    ...
    surfaceIds: z.array(z.string().min(1)).readonly().default([]),
    requirementIds: z.array(z.string().min(1)).readonly().default([]),
    testFiles: z.array(z.string().min(1)).readonly().default([]),
    generatedPages: z.array(z.string().min(1)).readonly().default([]),
    base: z.string().min(1),
