...
export function renderAgentInstructions(args: InstructionArgs): string {
  ...
  return [
    '## Documentation governance (docgen)',
    '',
    ...
    '**Required lifecycle:**',
    '',
    `- Before planning or editing, run \`${run} session start --json\`.`,
    `- After each coherent code edit, run \`${run} session after-edit --json\`.`,
    `- Before handing work back, run \`${run} session end --json\` and resolve failures.`,
    '- When impact reaches a feature, update or create its governed plan/change record.',
    ...
    '',
    '```sh',
    `${run} session start --json    # index, active plans, and open questions`,
    `${run} session after-edit --json # incremental index and change impact`,
    `${run} session end --json      # sync, tester handoff, and deterministic check`,
